...
public class Jaxb2SoapEncoder implements Encoder<Object> {
    ...
    private Flux<DataBuffer> encode(Object value ,
        ...
        return Mono.fromCallable(() -> {
            ...
            try {
                SoapRequest soapRequest = (SoapRequest)value;

                OutputStream outputStream = buffer.asOutputStream();
                Class<?> clazz = ClassUtils.getUserClass(soapRequest.getBody());
                ...
                if( soapRequest.getHeaderContent() != null ){
                    SoapMessage soapMessage = (SoapMessage)message;
                    SoapHeader header = soapMessage.getSoapHeader();
                    StringSource headerSource = new StringSource(soapRequest.getHeaderContent());
                    Transformer transformer = TransformerFactory.newInstance().newTransformer();
                    transformer.transform(headerSource, header.getResult());
                }

                marshaller.marshal(soapRequest.getBody(), message.getPayloadResult());
